...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        # Search for the user
        results = list(users_container.query_items(query="SELECT users.name FROM users WHERE users.email = @target1 AND users.password = @target2", parameters=[{"name": "@target1", "value": email}, {"name": "@target2", "value": password}], enable_cross_partition_query=True))
        ...
        return func.HttpResponse(body = json.dumps(results[0]['name']), status_code=200)
