...
class TestFunction(unittest.TestCase):
    ...
    def test_correct(self):
        payload = {"email1": "example1@example.com" , "email2" : "example2@example.com"}
        resp = requests.get('http://localhost:7071/api/user/addfriend', json = payload)
        self.assertEqual({"result" : True, 'message' : 'OK'}, resp.json())

        payload = {"email2": "example1@example.com", "email1": "example2@example.com"}
        resp = requests.get('http://localhost:7071/api/user/acceptfriend', json = payload)
        self.assertEqual({"result" : True, 'message' : 'OK'}, resp.json())

        payload = {"email1": "example1@example.com", "password": "12345678","email2": "example2@example.com"}
        resp = requests.get('http://localhost:7071/api/user/deletefriend', json = payload)
        self.assertEqual({"result" : True, 'message' : 'OK'}, resp.json())

    def test_correct_cancel_request(self):
        payload = {"email1": "example1@example.com" , "email2" : "example2@example.com"}
        resp = requests.get('http://localhost:7071/api/user/addfriend', json = payload)
        self.assertEqual({"result" : True, 'message' : 'OK'}, resp.json())
