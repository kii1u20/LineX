...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Rejecting friend request')

    request = req.get_json()

    #the person trying to reject the request
    ...
    try:
        ...
        if(len(result2) == 0):
            ...
            return func.HttpResponse(body = json.dumps({"result": False, "message": "The user you are trying to reject as a friend has not registered yet" }), status_code=400)
        ...
        if(any(x['friend'] == email_pending and x['status'] == 'friend' for x in result1[0]['friends'])):
            return func.HttpResponse(body = json.dumps({"result": False, "message": "You are friend with this user, maybe you'd like to remove them" }), status_code=400)
        ...
        result1[0]['friends'] = [friend for friend in result1[0]['friends'] if friend['friend'] != email_pending]
